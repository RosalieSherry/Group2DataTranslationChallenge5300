---
title: "Group 2 - Data Translation ChallegProject Code"
author: "Lynna Tran"
date: "3/19/2021"
...
binaryemployedretail_df <- employment_displaced_df %>%
  ...
  mutate(retail_employed = employed_binary == 1 & retail_binary == 1) %>%
  mutate(retail_employed_binary = ifelse(retail_employed == TRUE, 1, 0)) ##%>%
  ##mutate(employment_mean = mean(employed_binary))


# dim(binaryemployedretail_df)
# ##4397806 rows 
# 
##binaryemployedretail_groupdf <-  binaryemployedretail_df %>%
  ##group_by(employed) %>%
  ##summarise(mutate(employment_mean = mean(employed_binary)))



mean_employmentregression <- lm(employed_binary ~ 1, data = binaryemployedretail_df)

summary(mean_employmentregression)


ggplot(data = mean_employmentregression, aes( x= employed_binary, y = 1)) + geom_point() + geom_smooth(method= 'lm')
...
hist(rstandard(mean_employmentregression), 
...
summary(employment_on_retailregression)


ggplot(data = employment_on_retailregression, aes( x= employed_binary)) + geom_point() + geom_smooth(method= 'lm')



hist(rstandard(employment_on_retailregression), 
     xlab = "Standardized residuals", main = 'Standardized Residuals of ')
...
summary(employmentretailregressionCOVID)


ggplot(data = employmentretailregressionCOVID, aes( x= employed_binary, y = (retail_noNA * afterCovid))) + geom_point() + geom_smooth(method= 'lm')



hist(rstandard(employmentretailregressionCOVID), 
     xlab = "Standardized residuals", main = 'Standardized Residuals of ')
...
# otherindustriesregressionCOVID <- lm(employed_binary ~ 1 + retail_noNA + afterCovid + artind +
#                                   constructionind + eduind
#                                  + financeind + infoind + otherind
#                                  + proind + publicind + transportind
#                                  + wsind, data = otherindustries_df)
# 
# 
# summary(otherindustriesregressionCOVID)


otherindustriesregressionCOVID <- lm(employed_binary ~ 1 + (retail_noNA * afterCovid) + 
                                    (artind *    afterCovid) +
...
ggplot(data = otherindustriesregressionCOVID, aes( x= employed_binary))) + geom_point() + geom_smooth(method= 'lm')



hist(rstandard(otherindustriesregressionCOVID), 
     xlab = "Standardized residuals", main = 'Standardized Residuals of ')
