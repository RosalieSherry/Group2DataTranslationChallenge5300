...
##Data from IPUMS
ddi <- read_ipums_ddi("data/Lynna-Data/cps_00001.xml")
...
##IndustryCodes
RetailIndustryCodes20142019 <- read_csv('data/Lynna-Data/20142019IndustryCodes.csv')

RetailIndustryCodes2020 <- read_csv('data/Lynna-Data/2020IndustryCodes.csv')
```
...
# work_df <- work_df %>%
#   mutate(household_personid = (YEAR + MONTH + SERIAL + PERNUM, sep="-"))
...
employment_df <- inner_join(work_df, jobtenure_df, by = 'household_personid') %>%
  select(YEAR.x, SERIAL.x, MONTH.x, SERIAL.x, PERNUM.x, household_personid, ##shared variables
         ##work_df variables
         unemployed, 
